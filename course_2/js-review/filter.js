...
//Using filter to get books with more than 400 pages
let longBook = booksAPI.filter((books) => books.pages > 400);
let longBookWithAdaptation = booksAPI.filter(
  (book) => (book.pages > 400) & (book.hasMovieAdaptation === true)
);

//Filtering books by genre
let fantasyBooks = booksAPI.filter((book) => book.genres.includes("Fantasy"));

//Filtering a previous list
let finalBooksList = longBookWithAdaptation.filter((longBook) =>
  longBook.genres.includes("Fantasy")
);
